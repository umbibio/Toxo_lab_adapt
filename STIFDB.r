...
UV <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/UVB.txt", sep = "\t") 
UV.df <- UV %>% dplyr::transmute(GeneID = V2) %>% mutate(UV = "UV")

HE <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/Heat.txt", sep = "\t") 
HE.df <- HE %>% dplyr::transmute(GeneID = V2) %>% mutate(HE = "HE")

OSM <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/Osmotic.txt", sep = "\t") 
OSM.df <- OSM %>% dplyr::transmute(GeneID = V2) %>% mutate(OSM = "OSM")

IRON <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/IRON.txt", sep = "\t") 
IR.df <- IRON %>% dplyr::transmute(GeneID = V2) %>% mutate(IR = "IR")

ALUM <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/AL.txt", sep = "\t") 
AL.df <- ALUM %>% dplyr::transmute(GeneID = V2) %>% mutate(AL = "AL")


WOUND <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/WOUND.txt", sep = "\t") 
WO.df <- WOUND %>% dplyr::transmute(GeneID = V2) %>% mutate(WO = "WO")


ABA <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/ABA.txt", sep = "\t") 
ABA.df <- ABA %>% dplyr::transmute(GeneID = V2) %>% mutate(AB = "AB")

COLD <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/COLD.txt", sep = "\t") 
CO.df <- COLD %>% dplyr::transmute(GeneID = V2) %>% mutate(CO = "CO")


DROUGHT <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/DROUGHT.txt", sep = "\t") 
DR.df <- DROUGHT %>% dplyr::transmute(GeneID = V2) %>% mutate(DR = "DR")

LIGHT <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/LIGHT.txt", sep = "\t") 
LI.df <- LIGHT %>% dplyr::transmute(GeneID = V2) %>% mutate(LI = "LI")

NaCI <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/NaCI.txt", sep = "\t") 
NaCI.df <- NaCI %>% dplyr::transmute(GeneID = V2) %>% mutate(NaCI = "NaCI")


OXIDATIV <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/OXIDATIVE.txt", sep = "\t") 
OX.df <- OXIDATIV %>% dplyr::transmute(GeneID = V2) %>% mutate(OX = "OX")

DEHYD <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/DEHYDRATION.txt", sep = "\t") 
DH.df <- DEHYD %>% dplyr::transmute(GeneID = V2) %>% mutate(DH = "DH")

CDS <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/cold_drought_salt.txt", sep = "\t") 
...
write.xlsx(tmp.df, "../Input/Toxo_lab_adapt/stress/arabidopsis_stress_genes_STIFDB_YR.xlsx")
...
plant.urls <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/URL_Stress_responsive_genes_predicted_TF_arabidopsis.txt", header = F)
...
#saveRDS(TF.Genes.list, "../Input/Toxo_lab_adapt/RDS/arabiidopsis_tf_genes_list.rds")
TF.Genes.list <- readRDS("../Input/Toxo_lab_adapt/RDS/arabiidopsis_tf_genes_list.rds")

Cis.urls <- read.table("../Input/Toxo_lab_adapt/stress/STIFDB/URL_TF_family_Cis_element_arabidopsis.txt", header = F)
...
#write.xlsx(cis.element.tf, "../Input/Toxo_lab_adapt/stress/arabidopsis_tf_cis_element_STIFDB.xlsx")


arab.tf.genes <- readRDS("../Input/Toxo_lab_adapt//RDS/arabiidopsis_tf_genes_list.rds")
...
cis.element.tf <- read.xlsx("../Input/Toxo_lab_adapt/stress/STIFDB/arabidopsis_tf_cis_element_STIFDB.xlsx")
arab.stress.genes <- read.xlsx("../Input/Toxo_lab_adapt/stress/STIFDB/arabidopsis_stress_genes_STIFDB_YR.xlsx")
...
saveRDS(arab.genes.strs.tf.cis, "../Input/Toxo_lab_adapt/RDS/arabidopsis_stress_genes_STIFDB.rds")
...
yeast <- read.xlsx("../Input/Toxo_lab_adapt/stress/stress_related_genes_saccharomyces.xlsx", colNames  =T)
yeast <- data.frame(yeast$Gene.Systematic.Name)
write.table(yeast, "../Input/Toxo_lab_adapt/stress/YeastGenome/stress_related_genes_saccharomyces_IDs.txt", row.names = F, 
            col.names = F, 
            quote = FALSE)
